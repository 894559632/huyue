    ...
    <dd>
      <div class="rule-multi-radio">
        <span id="rblStatus">
            ...
            <label>正常</label>
            <input type="radio" name="isEnable" value="0" datatype="n" <#if !(user?? && user.isEnable?? && user.isEnable == true)>checked="checked"</#if>>
            <label>禁用</label>
        </span>
      </div>
    ...
    <dt>所属城市</dt>
        <dd>
            <div class="rule-single-select">
                <select name="cityId" id="categoryId" datatype="*" sucmsg=" ">
                    <#if city_list??>
  ...
  <dl>
    ...
    <dd>
      ...
      <dl>
        <dt>积分</dt>
        <dd>
            <input name="point" type="text" value="<#if user??&&user.point??>${user.point?string("0.00")}<#else>0.00</#if>" class="input small" datatype="/^-?\d+\.?\d{0,2}$/" sucmsg=" " errormsg="请输入2位小数">
      ...
      <dl>
        <dt>排序数字</dt>
        <dd>
            <input name="sortId" type="text" value="<#if user??>${user.sortId!"0"}<#else>99</#if>" class="input small" datatype="/^(([1-9]\d{0,1})|0)((\.\d{2})|(\.\d{1}))?$/" sucmsg=" " errormsg="请输入不超过100的2位小数">
            <span class="Validform_checktip">*数字，越小越向前</span>
        </dd>
    </dl>    
</div>
