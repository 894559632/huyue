                    ...
                    <#if order??>
                        <#list order.orderGoodsList as og>
                            <tr>
                                ...
                                <td><#if og.pointUse??>${og.pointUse?string("0.00")}<#else>0.00</#if></td>
                                <td>${og.quantity!'0'}</td>
                                <td>${(og.price*og.quantity)?string("0.00")}</td>
                                <td><#if og.pointUse??>${(og.pointUse*og.quantity)?string("0.00")}<#else>0.00</#if></td>
                            </tr>
                        </#list>
                    </#if>
                    </tbody>
                ...
                <td colspan="3" style="border-top: 1px solid #000;">
                    ...
                    <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" style="border-top: 1px solid #000; font-size: 12px; font-family: '微软雅黑'; background: #fff;">
                        ...
                        <#if order.orderNumber?contains("JFDH")>
                        	<tr>
	                            <td align="right">消费积分：￥${order.pointUse?string("0.00")}
	                                = 实际消费：${order.pointUse?string("0.00")}</td>
	                        </tr>
                        <#else>
	                        <tr>
