...
base.cart.selectCart = function(cartId) {
	...
	if (selected) {
	    ...
	    for (var i = 0; i < selected.length; i++) {
		var id = selected[i];
		if (id && "" !== id && Number(id) === Number(cartId)) {
		    isAdd = false;
		} else {
...
// 删除购物车项的方法
base.cart.remove = function(cartId) {
    if (cartId) {
	$.ajax({
	    url : "/touch/cart/remove",
	    type : "POST",
	    data : {
		cartId : cartId
	    },
		...
		if (0 !== res.status) {
		    ...
		    var container = base.getE("container" + cartId);
		    container.parentNode.removeChild(container);

		    // 判断其是否在已选中
		    var new_selected = [];
		    for (var i = 0; i < base.cart.selected.length; i++) {
			var id = base.cart.selected[i];
			if (id && "" !== id && Number(id) !== Number(cartId)) {
			    new_selected.push(id);
			}
		    }
